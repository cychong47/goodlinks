...
class Obsidian:
    def __init__(self, req_date):
        ...
        self.dn_file = f"""{os.environ["HOME"]}/Library/Mobile Documents/iCloud~md~obsidian/Documents/Notes/09. Daily Notes/{req_date}.md"""
        ...
        with open(self.dn_file, "r") as fp:
            for line in fp.readlines():
                if "## Goodlinks" in line:
                    print(
                        f"{self.req_date} : Daily Notes has already Goodlinks section."
                    )
                    return True
